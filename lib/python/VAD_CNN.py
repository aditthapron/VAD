...
def inference(inputs, keep_prob, is_training=True, reuse=None):
    ...
    with tf.variable_scope(scope_name):
        ...
        X = tf.expand_dims(in_rnn, -1)
        X = tf.layers.conv2d(inputs=X,filters=128,kernel_size=[5, 5],padding="same",activation=tf.nn.relu)
        X = tf.layers.batch_normalization(X, training=is_training)
        X = tf.nn.dropout(X, keep_prob=keep_prob)
        X = tf.layers.conv2d(inputs=X,filters=64,kernel_size=[3, 3],padding="same",activation=tf.nn.relu)
        X = tf.layers.batch_normalization(X, training=is_training)
        X = tf.nn.dropout(X, keep_prob=keep_prob)
        X = tf.layers.conv2d(inputs=X,filters=32,kernel_size=[3, 3],padding="same",activation=tf.nn.relu)
        X = tf.layers.batch_normalization(X, training=is_training)
        outputs = tf.nn.dropout(X, keep_prob=keep_prob)
        outputs = tf.reshape(outputs,[-1,target_delay+seq_size,num_features*32])
        outputs = tf.layers.conv1d(inputs=outputs,filters=16,kernel_size=3,padding="same",activation=tf.nn.relu)
        outputs = tf.layers.batch_normalization(outputs, training=is_training)
        outputs = tf.reshape(outputs[:, 0:seq_size, :],[-1,16])
...
def main(prj_dir=None, model=None, mode=None):
    ...
    if mode is 'train':
        ...
        sys.path.insert(0, prj_dir+'/configure/CNN')
        import config as cg
    ...
    ckpt = tf.train.get_checkpoint_state(logs_dir + '/CNN')
    ...
    if ckpt and ckpt.model_checkpoint_path:  # model restore
        ...
        if mode is 'train':
            ...
            saver.save(sess, logs_dir + "/model_CNN.ckpt", 0)  # model save
